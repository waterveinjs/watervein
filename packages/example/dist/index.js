...
const initialProcesses = Array.from({ length: 100 }, (_, i) => ({
    pid: 1000 + i,
    name: `korphere-worker-${i.toString().padStart(2, '0')}`,
    cpu: Math.floor(Math.random() * 20),
    memory: Math.floor(Math.random() * 200)
}));
const processes = createState(initialProcesses);
const sortBy = createState("pid");
...
const app = element("div", { class: "monitoring-dashboard", style: { padding: "20px", fontFamily: "sans-serif" } }, [
    ...
    element("section", { class: "panel spec-panel" }, [
        ...
        Show(() => read(serverSpecResource).loading, () => element("div", { class: "loading" }, "Fetching remote hardware details via stream pipeline..."), () => {
            const res = read(serverSpecResource);
            if (res.error)
                return element("div", {}, `Error: ${res.error.message}`);
