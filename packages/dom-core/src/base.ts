...
import type { InternalDOM } from './internal.js';
  ...
  const e = createEffect(() => {
    ...
    try {
      ...
      if (isInitial) {
        ...
        if (count > 0) {
          if (scratch.srcBuf.length < count) {
            scratch.srcBuf = new Int32Array(
              Math.max(count, scratch.srcBuf.length * 2),
            );
          }
      ...
      if (scratch.srcBuf.length > 64 && newLen < scratch.srcBuf.length >> 2) {
        scratch.srcBuf = new Int32Array(
          Math.max(64, scratch.srcBuf.length >> 1),
        );
      }
