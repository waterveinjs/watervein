import type { Node as WvNode } from '@watervein/core';
import {
  createState,
  createCompute,
  createEffect,
...
import { element as el0, For as for0 } from '@watervein/dom-core';
...
const BenchmarkSystem = {
  runGraphConstruction(nodeCount: number) {
    ...
    withEntity(rootEntity, () => {
      ...
      for (let i = 0; i < nodeCount / 2; i++) {
        ...
        const nextRight = createCompute(
          () => read(currentRight) + read(currentLeft),
        );
  ...
  runForReorderPerformance(itemCount: number = 1000) {
    ...
    const container = for0(
      itemsState,
      (item: { id: number; text: string }) => item.id,
      (item) => {
    ...
    const reversedData = [
      ...read<{ id: number; text: string }[]>(itemsState),
    ].reverse();
  ...
  runDslStaticVsDynamic(count: number) {
    ...
    for (let i = 0; i < count; i++) {
      const el = el0(
        'div',
        { class: 'box static-mode', style: { color: 'red', margin: '10px' } },
        ['Static Text'],
      );
      container.appendChild(el);
    }
    ...
    return {
      ...
      dynamicTime: endDynamic - startDynamic,
    };
  },

  runForListReset(itemCount: number) {
    type Row = { id: number; text: string };
    const initialData = Array.from({ length: itemCount }, (_, i) => ({
      id: i,
      text: `Item ${i}`,
    }));
...
async function runScaleSuite() {
  ...
  for (const level of SCALE_LEVELS) {
...

    {
      ...
      console.log(
        `[BUILD DAG] ${time.toFixed(2)}ms | ${formatMemoryDelta(beforeAfterGC, after)}`,
      );
...

    {
      ...
      const time = BenchmarkSystem.runHighFrequencyUpdate(
        stateCount,
        iterations,
      );
      console.log(
        `[HIGH FREQUENCY UPDATE] state=${stateCount} iter=${iterations}: ${time.toFixed(2)}ms`,
      );
    }
    await yieldMainThread();

    {
      ...
      const batchedTime =
        BenchmarkSystem.runMassDestructionBatched(entityCount);
      console.log(
        `[DESTROY ENTITY] individual=${individualTime.toFixed(2)}ms / batched=${batchedTime.toFixed(2)}ms (${(individualTime / batchedTime).toFixed(1)}x)`,
      );
    }
    await yieldMainThread();

    {
      ...
      const after = getMemory();
      console.log(
        `[PARTIAL REPLACE] 10% replace=${time10.toFixed(2)}ms / 50% replace=${time50.toFixed(2)}ms | ${formatMemoryDelta(before, after)}`,
      );
    }
    await yieldMainThread();

    {
      ...
      console.log(
        `[Fan-Out/In] n=${fanCount}: out=${fanOutTime.toFixed(2)}ms in=${fanInTime.toFixed(2)}ms`,
      );
    }
    await yieldMainThread();

    {
      ...
      const { time: diamondTime } =
        BenchmarkSystem.runDiamondProblem(diamondWidth);
      console.log(
        `[DIAMOND PROBLEM] width=${diamondWidth}: ${diamondTime.toFixed(2)}ms`,
      );
    }
    await yieldMainThread();

    {
      ...
      const cleanupTime =
        BenchmarkSystem.runUnusedEdgeCleanup(cleanupIterations);
      console.log(
        `[UNUSED EDGE CLEANUP] iter=${cleanupIterations}: ${cleanupTime.toFixed(2)}ms`,
      );
    }
    await yieldMainThread();

    {
      ...
      console.log(
        `[FOR LIST RESET] items=${listResetCount}: ${resetTime.toFixed(2)}ms`,
      );
    }
    await yieldMainThread();

    {
      ...
      const after = getMemory();
      console.log(
        `[CREATE COST] state=${stateTime.toFixed(2)}ms compute=${computeTime.toFixed(2)}ms effect=${effectTime.toFixed(2)}ms | ${formatMemoryDelta(before, after)}`,
      );
    }
    await yieldMainThread();
  }
...
(window as any).runWaterveinBenchmark = async () => {
  ...
  const { time: constTime, entityId } =
    BenchmarkSystem.runGraphConstruction(10000);
  console.log(
    `[1] Building a Massive DAG with 10,000 Nodes: ${constTime.toFixed(2)} ms`,
  );
  DestructionSystem.destroyEntity(entityId);
  await yieldMainThread();
  ...
  console.log(
    `[2] 2,000 signals x 50 batch updates (100,000 patches total): ${updateTime.toFixed(2)} ms`,
  );
  await yieldMainThread();

  const destroyTime = BenchmarkSystem.runMassDestruction(5000);
  console.log(
    `[3] Batch Memory Release for 5,000 Entities (10,000 Nodes) (Individually): ${destroyTime.toFixed(2)} ms`,
  );
  await yieldMainThread();

  const destroyBatchedTime = BenchmarkSystem.runMassDestructionBatched(5000);
  console.log(
    `[3b] Batch memory release for 5,000 entities (10,000 nodes): ${destroyBatchedTime.toFixed(2)} ms`,
  );
  await yieldMainThread();

  const dsl0Time = BenchmarkSystem.runDsl0Construction(1000);
  console.log(
    `[DOM-CORE] Mounting 1,000 elements using raw DOM (raw arrow functions): ${dsl0Time.toFixed(2)} ms`,
  );
  await yieldMainThread();

  const reorderTime = BenchmarkSystem.runForReorderPerformance(1000);
  console.log(
    `[For Optimization] Batch Reverse Sort (Reorder) of 1,000 DOM Elements: ${reorderTime.toFixed(2)} ms`,
  );
  await yieldMainThread();
  ...
  console.log(
    `[Partially Replace] 1,000 out of 1,000 elements replaced (10%): ${replace10.toFixed(2)} ms / 50%: ${replace50.toFixed(2)} ms`,
  );
  await yieldMainThread();

  console.log(
    `[4] Dynamic Dependency: ${BenchmarkSystem.runDynamicDependency(100000).toFixed(2)} ms`,
  );
  await yieldMainThread();
  console.log(
    `[5] Fan-Out (1 -> 10000): ${BenchmarkSystem.runFanOutBenchmark(10000).toFixed(2)} ms`,
  );
  await yieldMainThread();
  console.log(
    `[6] Fan-In (10000 -> 1): ${BenchmarkSystem.runFanInBenchmark(10000).toFixed(2)} ms`,
  );
  await yieldMainThread();
  console.log(
    `[7] Deep Chain (10000): ${BenchmarkSystem.runDeepChain(1000).toFixed(2)} ms`,
  );
  await yieldMainThread();
  console.log(
    `[8r] Read (10000): ${BenchmarkSystem.runRead(10000).toFixed(2)} ms`,
  );
  await yieldMainThread();
  console.log(
    `[8w] Write (10000): ${BenchmarkSystem.runWrite(10000).toFixed(2)} ms`,
  );
  await yieldMainThread();
  console.log(
    `[9] Flush (10000): ${BenchmarkSystem.runFlush(10000).toFixed(2)} ms`,
  );
  await yieldMainThread();
  console.log(
    `[10s] create-state (10000): ${BenchmarkSystem.runCreateState(10000).toFixed(2)} ms`,
  );
  await yieldMainThread();
  console.log(
    `[10c] create-compute (10000): ${BenchmarkSystem.runCreateCompute(10000).toFixed(2)} ms`,
  );
  await yieldMainThread();
  console.log(
    `[10e] create-effect (10000): ${BenchmarkSystem.runCreateEffect(10000).toFixed(2)} ms`,
  );
  await yieldMainThread();

  const diamondRes = BenchmarkSystem.runDiamondProblem(2000);
  console.log(
    `[11] Diamond Problem (Width 2,000): ${diamondRes.time.toFixed(2)} ms (Glitch Free: ${diamondRes.glitchesPrevented})`,
  );
  await yieldMainThread();

  const cleanupTime = BenchmarkSystem.runUnusedEdgeCleanup(5000);
  console.log(
    `[12] Dynamic Edge Cleanup (5,000 toggles): ${cleanupTime.toFixed(2)} ms`,
  );
  await yieldMainThread();

  const redundantTime = BenchmarkSystem.runRedundantWriteFiltering(50000);
  console.log(
    `[13] Redundant Write Filtering (50,000 identical writes): ${redundantTime.toFixed(2)} ms`,
  );
  await yieldMainThread();

  const dslProps = BenchmarkSystem.runDslStaticVsDynamic(1000);
  console.log(
    `[14] DSL Property parsing (1,000 el) -> Static: ${dslProps.staticTime.toFixed(2)} ms / Dynamic: ${dslProps.dynamicTime.toFixed(2)} ms`,
  );
  await yieldMainThread();

  const listResetTime = BenchmarkSystem.runForListReset(1000);
  console.log(
    `[15] For-Loop List Clear & Re-populate (1,000 elements): ${listResetTime.toFixed(2)} ms`,
  );
  await yieldMainThread();

  const itemPropTime = BenchmarkSystem.runForItemPropUpdate(1000, 5000);
  console.log(
    `[16] For-Loop Single Item Property Update (5,000 writes): ${itemPropTime.toFixed(2)} ms`,
  );
  await yieldMainThread();

  const crossEntityTime = BenchmarkSystem.runCrossEntityDependency(2000);
  console.log(
    `[17] Cross-Entity Dependency & Cascade Destroy (2,000 entities): ${crossEntityTime.toFixed(2)} ms`,
  );
