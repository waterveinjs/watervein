...
const DEFAULT_EDGE_CAPACITY = 256;
...
export function initCapacity(options: { edgeCapacity?: number }): void {
  if (edgePoolInitialized) {
    throw new Error(
      '[watervein] initCapacity() must be called before any node or edge is created, and can only be called once.',
    );
  }
  if (nextUnallocatedEdgeId !== 0 || NODE_ID_COUNTER !== 0) {
    throw new Error(
      '[watervein] initCapacity() must be called before any node or edge is created.',
    );
  }
...
function expandInt32<T extends ArrayBufferLike>(
  old: Int32Array<T>,
  newCap: number,
): Int32Array<T> {
...
export function N(id: number): Node {
  ...
  if (!node || node.id === -1) {
    throw new Error(
      `[watervein] Node with id ${id} is undefined or destroyed.`,
    );
  }
  return node;
}
...
let ENTITY_COUNT = 0;
const entityRegistry: (Node[] | undefined)[] = [];
...
let raFID: NextTickId | null = null;
...
export function registerCustomNodeType(): number {
...
function linkEdge(dep: Node, sub: Node): number {
  if ((dep.ft & FLAG_DISPOSED) !== 0 || (sub.ft & FLAG_DISPOSED) !== 0)
    return NULL_EDGE;
...
function commitEdges(sub: Node) {
  ...
  try {
    ...
    while (edgeId !== NULL_EDGE) {
      ...
      if (
        depNode &&
        (depNode.ft & FLAG_DISPOSED) === 0 &&
        depNode.watchedVersion === pendingStamp
      ) {
...
type NextTickId = number | ReturnType<typeof setTimeout>;
const nextTick =
  typeof requestAnimationFrame !== 'undefined'
    ? (cb: NextTickCallback) => requestAnimationFrame(cb)
    : (cb: NextTickCallback) => setTimeout(cb, 0);
...
function handleFlushError(node: Node, err: any) {
  if (isDev) {
    console.error(
      `[watervein-error] Exception caught during flush (Node ID: ${node.id}, Type: ${node.ft >>> TYPE_SHIFT}).\n` +
...
export function createCompute<T>(fn: () => T): Node<T> {
  const node: Node<T> = createNode<T>(
    NODE_TYPE_COMPUTE,
    undefined as any,
    () => {
      if (isDev && evaluationStack) {
        if (evaluationStack.has(node.id))
          throw new Error(
            `[watervein] Circular reference on compute ${node.id}`,
          );
...
export function createEffect(fn: () => void): Node<void> {
  const node: Node<void> = createNode<void>(NODE_TYPE_EFFECT, undefined, () => {
    if (isDev && evaluationStack) {
      if (evaluationStack.has(node.id))
        throw new Error(`[watervein] Circular reference on effect ${node.id}`);
      evaluationStack.add(node.id);
    }
    pushTrackingNode(node);
    try {
...
export function createResource<S, T>(
  ...
  const resourceNode = createNode<ResourceResult<T>>(NODE_TYPE_STATE, {
    data: undefined,
    loading: true,
    error: null,
  });
...
export function read<T>(node: Node<T>): T {
  if (isDev && !isNode(node)) {
    throw new Error(
      '[watervein] read() was called with a value that is not a reactive Node.',
    );
  }
  if (isDev && (node.ft & FLAG_DISPOSED) !== 0) {
    console.warn(
      `[watervein] read() called on a disposed node. Returning stale value.`,
    );
  }
  const trk = currentTrackingNode;
...
function collectRecursively(id: number, allTargets: Set<number>) {
    ...
    try {
      ...
      for (const eId of allTargetEntityIds) {
        ...
        if (
          parentId !== undefined &&
          parentId !== null &&
          !allTargetEntityIds.has(parentId)
        ) {
          const parentChildren = entityChildrenMap.get(parentId);
          if (parentChildren) {
      ...
      const willFreeAll =
        freeEntityIds.length + allTargetEntityIds.size === ENTITY_COUNT;
      if (willFreeAll) {
        ENTITY_COUNT = 0;
  ...
  _cleanupNode(node: Node) {
    ...
    if (nodeType === NODE_TYPE_EFFECT && typeof node.value === 'function') {
      try {
        ...
        console.error(
          `[watervein] Error during effect cleanup on node ${node.id}:`,
          err,
        );
      }
    }
    ...
    if (node.cleanups !== null) {
      ...
      for (let i = 0; i < list.length; i++) {
        try {
          ...
          if (isDev)
            console.error(
              `[watervein] Error during onCleanup callback on node ${node.id}:`,
              err,
            );
        }
      }
